...
**The hash covers the body, never the frontmatter.** A leading `---` block is removed whole before hashing, and so are bare declaration lines at the top of a file that has no block, the older form, and any blank lines left after either. What remains is what the person wrote and what a compiled note depends on.

The reason is that **metadata is not content**. Tagging a source, writing its backlinks, retitling it or retagging it for a folder changes no words. If the hash covered those lines, each such keystroke would mark every note compiled from that source stale and rebuild all of them for nothing. Until v2026-09-24 only five named lines were excluded, which made every metadata write a hazard: a system write had to be a byte-exact line insert, and giving a bare-form file a fenced block shifted its hash. Excluding the block whole removed the hazard rather than guarding it.

**The body is hashed byte for byte.** Line endings and trailing whitespace count, so normalising CRLF to LF or reflowing a paragraph changes the hash exactly as editing a word would. A system write therefore stays inside the frontmatter and never reads and rewrites the whole file in text mode, which can silently convert the body's line endings; within the block it is free to add a key, or to give a bare-form file a block, since none of that reaches the hash.

**The canonical implementation is pinned** at `scripts/hash_source.py`. A convenience reimplementation is a trap: Python's `str.splitlines`, for one, also breaks on a bare carriage return and several Unicode separators, so it disagrees on many real files and reports staleness that is not there. Use the shipped hasher. A vault compiled under the old rule is re-stamped once with `scripts/migrate_hash_v2.py`, which refuses while any source has drifted.
